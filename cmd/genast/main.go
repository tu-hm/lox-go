...
//
// The token import is emitted unconditionally. That is correct for Expr; when
// chapter 8 adds Stmt, either every hierarchy references token.Token somewhere
// (Var does, for its name) or this needs to compute the import set from the
// field types. Unused imports are a compile error in Go, so format.Source will
// tell you the moment it matters.
